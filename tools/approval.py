...
def _is_cron_approval_context() -> bool:
    """True only for the current scheduled-job context.

    Keep the legacy environment fallback through ``is_cron_session`` for
    standalone schedulers/tests, while embedded gateway jobs use a ContextVar
    so cron classification cannot persist into unrelated conversations.
    """
    try:
