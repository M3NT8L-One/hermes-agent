...
class TestAtomicSnapshotWrite:
    ...
    def test_temp_path_uses_mktemp_template(self):
        """The temp name must come from mktemp, not a shell PID variable.

        macOS Bash 3.2 has no BASHPID and ``$$`` is shared by background
        subshells, so either PID spelling can collapse concurrent writers onto
        one temp path.
        """
        env = _TestableEnv()
        env._snapshot_ready = True
    ...
    def test_temp_template_with_spaces_is_quoted(self):
        ...
        ``C:/Users/...`` or spaces) as one mktemp template argument."""
        env = _TestableEnv()
        env._snapshot_ready = True
    ...
    def test_init_session_bootstrap_also_atomic_and_unique(self):
        ...
        file a concurrent command could source — it must use the same atomic
        mktemp-and-move contract."""
        env = _TestableEnv()
        captured = {}
...
class TestAtomicSnapshotConcurrencyBehavioral:
    ...
    that would corrupt PATH. The writer uses mktemp so this guarantee also
    holds on macOS Bash 3.2, where BASHPID is unset.
    """
    ...
    def test_concurrent_writes_never_tear_the_snapshot(self, tmp_path):
        ...
        writer = (
            ...
            "2>/dev/null || rm -f \"$__hermes_snap_tmp\" "
            "2>/dev/null || true; fi; "
            "done"
        )
    ...
    def test_failed_export_does_not_destroy_good_snapshot(self, tmp_path):
        ...
        bad_tmp_template = _q("/nonexistent-dir/snap.tmp.XXXXXX")
        script = (
            f"if __hermes_snap_tmp=$(mktemp {bad_tmp_template}); then "
            f"{{ export -p > \"$__hermes_snap_tmp\" && "
            f"mv -f \"$__hermes_snap_tmp\" {_q(snap)}; }} "
            "2>/dev/null || rm -f \"$__hermes_snap_tmp\" "
            "2>/dev/null || true; fi"
