...
class TestHostHeaderValidator:
    ...
    def test_loopback_bind_accepts_configured_tailnet_proxy_host(self, monkeypatch):
        """Trusted local reverse proxies may preserve their Tailnet Host header."""
        from hermes_cli.web_server import _is_accepted_host

        monkeypatch.setenv(
            "HERMES_DASHBOARD_ALLOWED_HOSTS",
            "macstudio.tailda2347.ts.net,macstudio",
        )

        assert _is_accepted_host("macstudio.tailda2347.ts.net", "127.0.0.1")
        assert _is_accepted_host("macstudio.tailda2347.ts.net:443", "127.0.0.1")
        assert _is_accepted_host("macstudio", "127.0.0.1")
